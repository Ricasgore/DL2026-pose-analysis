    ...
    for epoch in range(num_epochs):
        for i, (images, labels) in enumerate(train_loader):
            ...
            if (i + 1) % 2 == 0:
                ...
                np.save('/home/amine_tsp/DL2026/Method_Automatic_pose_recognition/loss_save_during_training/lossCNN_Basic_datasetMatisse.npy',losses)
...
def plott():
    a = np.load('/home/amine_tsp/DL2026/Method_Automatic_pose_recognition/loss_save_during_training/lossCNN_Basic_dataset2.npy', allow_pickle=True)
    plt.plot(a)
    #plt.savefig('/home/amine_tsp/DL2026/Method_Automatic_pose_recognition/loss_graph_plot/lossCNN_Basic_dataset2.png')
    plt.savefig('/home/amine_tsp/DL2026/Method_Automatic_pose_recognition/loss_graph_plot/lossCNN_Resnet50_dataset2.png')
    plt.close()
    print('finish')
...
if __name__ == '__main__':
    ...
    if (model,dataset) == (0,1):
        ...
        CNNI, input_size = get_pretrained_models(model_name = list_models[sel], num_classes= 42, freeze_prior = False, 
                                                use_pretrained = True)
        CNNI = CNNI.to(device)
        SavingName = '/home/amine_tsp/DL2026/Method_Automatic_pose_recognition/checkpoints/CNN-pose-Resnet50_dataset1.ckpt'
        ...
        CNNI, input_size = get_pretrained_models(model_name = list_models[sel], num_classes= 32, freeze_prior = False, 
                                                use_pretrained = True)
        CNNI = CNNI.to(device)
        SavingName = '/home/amine_tsp/DL2026/Method_Automatic_pose_recognition/checkpoints/CNN-pose-Resnet50_dataset2.ckpt'
    elif (model,dataset) == (0,3):
        CNNI = FC(inputNode=602112,hiddenNode = 256, outputNode=101).to(device)
        CNNI = CNNI.to(device)
    ...
    if dataset == 1 :
        ...
        PoseTrain = UCF101Dataset(baseDir='/home/amine_tsp/DL2026/Datasets/UCF101', transform=tr, theType='train', split=1)

        train_loader = torch.utils.data.DataLoader(dataset=PoseTrain,
                                                batch_size=batch_size,
                                                shuffle=True)
        
        PoseVal = UCF101Dataset(baseDir='/home/amine_tsp/DL2026/Datasets/UCF101', transform=tr, theType='val', split=1)

        val_loader = torch.utils.data.DataLoader(dataset=PoseVal,
                                                batch_size=batch_size,
                                                shuffle=True)

        PoseTest = UCF101Dataset(baseDir='/home/amine_tsp/DL2026/Datasets/UCF101', transform=tr, theType='test', split=1)
