...
if (model,dataset) == (0,1):
    ...
    CNNI = CNNI.to(device)
    pred_x, pred_y = predict_dataset1(model=CNNI, SavingName='./checkpoints/CNN-pose-Basic_dataset1.ckpt', image=image)

elif (model, dataset) == (0,2):
    CNNI = CNN(num_classes=32).to(device)
    CNNI = CNNI.to(device)
    pred_x, pred_y = predict_dataset2(model=CNNI, SavingName='/home/amine_tsp/DL2026/Method_Automatic_pose_recognition/checkpoints/CNN-pose-Basic_dataset2.ckpt', image=image)
    ...
    CNNI, input_size = get_pretrained_models(model_name = list_models[sel], num_classes= 42, freeze_prior = False, 
                                            use_pretrained = True)
    CNNI = CNNI.to(device)
    pred_x, pred_y = predict_dataset1(model=CNNI, SavingName='/home/amine_tsp/DL2026/checkpoints/CNN-pose-Resnet50_dataset1.ckpt', image=image)
    ...
    CNNI, input_size = get_pretrained_models(model_name = list_models[sel], num_classes= 32, freeze_prior = False, 
                                            use_pretrained = True)
    CNNI = CNNI.to(device)
    pred_x, pred_y = predict_dataset2(model=CNNI, SavingName='/home/amine_tsp/DL2026/Method_Automatic_pose_recognition/checkpoints/CNN-pose-Resnet50_dataset2.ckpt', image=image)
...
#plt.savefig('/home/amine_tsp/DL2026/Method_Automatic_pose_recognition/predict_one_image/test_plot_Basic_dataset2.png')
plt.savefig('/home/amine_tsp/DL2026/Method_Automatic_pose_recognition/predict_one_image/test_plot_Resnet50_dataset1.png')
plt.close()